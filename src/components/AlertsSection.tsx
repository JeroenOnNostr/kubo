...
/**
 * Shared alerts list — the kid → parent request inbox + future safety
 * notifications. Mounted both standalone (the legacy /parent/alerts page,
 * now reachable only as a redirect target) and inlined under the per-kid
 * Home dashboard (KUBO-185).
 *
 ...
 *
 * `variant` controls the empty state:
 *  - `'page'` — full-height centered empty card (standalone page feel).
 *  - `'home'` — a quiet one-line "No alerts" so the Home dashboard stays
 *    compact when nothing needs the parent.
 */
export function AlertsSection({
  ...
  if (alerts.length === 0) {
    if (variant === 'home') {
      return (
        <p className="text-[12px] text-muted-foreground px-1">No alerts right now.</p>
      );
    }
    return (
